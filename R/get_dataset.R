...
get_dataset <- function(dataset_name,
  ...
  # This base R approach is much more performant than the dplyr/purrr equivalent.
  # It unnests the list of type vectors, splits them by name, and then checks
  # which names have more than one unique type.
  flat_types <- do.call(c, unname(types))
  grouped_types <- split(unname(flat_types), names(flat_types))
