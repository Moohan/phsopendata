...
get_dataset <- function(
  ...
  # This is a much faster way of checking for columns that have multiple
  # types across all resources.
  # It works by unlisting the list into a vector of types,
  # splitting that vector by name,
  # then checking the length of the unique values in each element of the list
  all_types <- do.call(c, unname(types))
  grouped_types <- split(all_types, names(all_types))
  to_coerce <- names(
    grouped_types
  )[vapply(grouped_types, function(x) length(unique(x)) > 1, logical(1))]
