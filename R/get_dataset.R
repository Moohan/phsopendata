...
get_dataset <- function(dataset_name,
  ...
  #    Using `do.call(c, unname(types))` is a fast and robust way to do this.
  ...
  #    `vapply` is used for a fast, type-safe check.
  inconsistent_cols <- vapply(types_by_col, function(x) length(unique(x)) > 1, logical(1))
